...
| `cs/`, `es/`, `fr/`, `hu/`, `id/`, `it/`, `ms/`, `nl/`, `pl/`, `pt/`, `ro/`, `sv/`, `tr/`, `uk/`, `vi/` | Screenshots + Feature-Grafik dieser fünfzehn Sprachen (seit 23.09.2026); `es/` gilt für **beide** spanischen Einträge (`es-419` und `es-ES`), `pt/` für `pt-BR`, `nl/` für `nl-NL`, `pl/` für `pl-PL`, `cs/` für `cs-CZ`, `hu/` für `hu-HU`, `sv/` für `sv-SE` | wie oben |
