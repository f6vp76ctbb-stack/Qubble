...
| `bg/`, `cs/`, `da/`, `es/`, `fi/`, `fr/`, `hr/`, `hu/`, `id/`, `it/`, `ms/`, `nb/`, `nl/`, `pl/`, `pt/`, `ro/`, `sk/`, `sv/`, `tr/`, `uk/`, `vi/` | Screenshots + Feature-Grafik dieser einundzwanzig Sprachen (seit 23.09.2026); `es/` gilt für **beide** spanischen Einträge (`es-419` und `es-ES`), `pt/` für `pt-BR`, `nl/` für `nl-NL`, `pl/` für `pl-PL`, `cs/` für `cs-CZ`, `hu/` für `hu-HU`, `sv/` für `sv-SE`, `da/` für `da-DK`, `nb/` für `no-NO`, `fi/` für `fi-FI` | wie oben |
| `ja/`, `ko/`, `th/`, `zh/`, `zh_Hant/`, `ar/`, `hi/`, `el/` | Screenshots + Feature-Grafik Griechisch (`el/` → `el-GR`, griechische Buchstaben in Noto Sans), Japanisch (`ja-JP`), Koreanisch (`ko-KR`), Thai (`th`), Hindi (`hi/` → `hi-IN`), Chinesisch vereinfacht (`zh/` → `zh-CN`) und traditionell (`zh_Hant/` → `zh-TW` **und** `zh-HK`), Arabisch (`ar`, von rechts nach links gesetzt) — Texte in Noto Sans CJK/Thai/Arabic/Devanagari | wie oben |
