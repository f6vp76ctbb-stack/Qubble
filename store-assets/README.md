...
| `cs/`, `da/`, `es/`, `fi/`, `fr/`, `hu/`, `id/`, `it/`, `ms/`, `nb/`, `nl/`, `pl/`, `pt/`, `ro/`, `sk/`, `sv/`, `tr/`, `uk/`, `vi/` | Screenshots + Feature-Grafik dieser neunzehn Sprachen (seit 23.09.2026); `es/` gilt für **beide** spanischen Einträge (`es-419` und `es-ES`), `pt/` für `pt-BR`, `nl/` für `nl-NL`, `pl/` für `pl-PL`, `cs/` für `cs-CZ`, `hu/` für `hu-HU`, `sv/` für `sv-SE`, `da/` für `da-DK`, `nb/` für `no-NO`, `fi/` für `fi-FI` | wie oben |
