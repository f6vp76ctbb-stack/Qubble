...
| `bg/`, `ca/`, `cs/`, `da/`, `es/`, `fi/`, `fil/`, `fr/`, `hr/`, `hu/`, `id/`, `it/`, `ms/`, `nb/`, `nl/`, `pl/`, `pt/`, `ro/`, `sk/`, `sv/`, `sw/`, `tr/`, `uk/`, `vi/` | Screenshots + Feature-Grafik dieser vierundzwanzig Sprachen (seit 23.09.2026); `es/` gilt für **beide** spanischen Einträge (`es-419` und `es-ES`), `pt/` für `pt-BR`, `nl/` für `nl-NL`, `pl/` für `pl-PL`, `cs/` für `cs-CZ`, `hu/` für `hu-HU`, `sv/` für `sv-SE`, `da/` für `da-DK`, `nb/` für `no-NO`, `fi/` für `fi-FI` | wie oben |
| `ja/`, `ko/`, `th/`, `zh/`, `zh_Hant/`, `ar/`, `hi/`, `el/`, `he/`, `ur/` | Screenshots + Feature-Grafik Urdu (`ur`, von rechts nach links, Noto Sans Arabic), Hebräisch (`he/` → `iw-IL`, von rechts nach links, Noto Sans Hebrew), Griechisch (`el/` → `el-GR`, griechische Buchstaben in Noto Sans), Japanisch (`ja-JP`), Koreanisch (`ko-KR`), Thai (`th`), Hindi (`hi/` → `hi-IN`), Chinesisch vereinfacht (`zh/` → `zh-CN`) und traditionell (`zh_Hant/` → `zh-TW` **und** `zh-HK`), Arabisch (`ar`, von rechts nach links gesetzt) — Texte in Noto Sans CJK/Thai/Arabic/Devanagari | wie oben |
