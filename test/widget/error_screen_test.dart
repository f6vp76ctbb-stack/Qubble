...
void main() {
  testWidgets('shows a readable cause instead of a blank surface',
    ...
    expect(find.text('Hier ist etwas schiefgelaufen'), findsOneWidget);
    expect(
      find.textContaining('Board konnte nicht gezeichnet werden'),
      findsOneWidget,
    );
    expect(find.textContaining('ins Feedback kopieren'), findsOneWidget);
  ...
  testWidgets('offers no reset when none was wired', (tester) async {
    await tester.pumpWidget(ErrorScreen(details: _details()));
    expect(find.text('Spielstand zurücksetzen'), findsNothing);
  });
  ...
  testWidgets('the reset runs once and reports back', (tester) async {
    ...
    await tester.tap(find.text('Spielstand zurücksetzen'));
    await tester.pumpAndSettle();

    expect(calls, 1);
    expect(
      find.textContaining('Spielstand zurückgesetzt'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('a failing reset leaves the button usable again', (tester) async {
    ...
    expect(calls, 1);
    expect(find.text('Spielstand zurücksetzen'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
